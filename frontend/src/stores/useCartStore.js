...
export const useCartStore = create((set, get) => ({
  ...
  // Get cart items from the backend and store them
  getCartItems: async () => {
    try {
      const res = await axiosInstance.get("/cart");

      // Combine items with the same product ID and size to avoid duplicates
      const uniqueCartItems = res.data.reduce((acc, item) => {
  ...
  addToCart: async (product, size, quantity) => {
    try {
      const response = await axiosInstance.post("/cart", { productId: product._id, size, quantity });
      toast.success("Product added to cart");
      ...
      set((prevState) => {
        ...
        if (existingItemIndex !== -1) {
          ...
          newCart = [...prevState.cart, { ...product, size, quantity }];
        }
  ...
  removeFromCart: async (productId, size) => {
    try {
      await axiosInstance.delete(`/cart`, { data: { productId, size } });
      set((prevState) => ({
  ...
  updateQuantity: async (productId, size, quantity) => {
    try {
      ...
      await axiosInstance.put(`/cart/${productId}`, { size, quantity });
      set((prevState) => ({
        cart: prevState.cart.map((item) =>
          item._id === productId && item.size === size
            ? { ...item, quantity }
  ...
  updateProductStock: async (productId, size, quantity) => {
    ...
    try {
      await axiosInstance.put(`/inventory/update-stock/${productId}`, {
        size: Number(size),
        quantity: Number(quantity),
      });
      console.log("Stock updated for product ID:", productId);
